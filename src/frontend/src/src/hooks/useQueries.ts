...
export function useGetLoanDocuments(loanId: bigint | null) {
  const { actor } = useActor();
  return useQuery({
    queryKey: ["loanDocuments", String(loanId)],
    queryFn: async () => {
      if (!actor || loanId === null) return [];
      return actor.getLoanDocuments(loanId);
    },
    enabled: !!actor && loanId !== null,
  });
}
  ...
  return useMutation({
    ...
    onSuccess: (_result, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["loanDocuments", String(variables.loanId)],
      });
    },
  });
}
...
export function useDeleteLoanDocument() {
  ...
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (data: { docId: bigint; loanId: bigint }) => {
      if (!actor) throw new Error("No actor");
      return actor.deleteLoanDocument(data.docId);
    },
    onSuccess: (_result, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["loanDocuments", String(variables.loanId)],
